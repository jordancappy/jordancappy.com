...
var Game = function () {
  ...
  _createClass(Game, [{
    key: 'generateBoard',
    value: function generateBoard(wedges) {
      ...
      for (var x = 0; x < wedges; x++) {
        ...
        for (var y = 9; y >= 0; y--) {
          if (!this.board[x][9 - y]) this.board[x][9 - y] = { empty: true };

          var index = y + 1,
              endangle = Math.PI / (wedges / 2);

          var arc = _d2.default.svg.arc().innerRadius(index * 30).outerRadius((index + 1) * 30).startAngle(x * endangle).endAngle((x + 1) * endangle);

          var blockColor = this.board[x][9 - y].color ? this.board[x][9 - y].color : '';
          group.append('path').attr('transform', 'translate(' + window.innerWidth / 2 + ',' + window.innerHeight / 2 + ') ')
          //'rotate(' + degrees * x +'deg)')
          .attr('d', arc).attr('class', 'block ' + blockColor);
        }
      }

      this.positions = _d2.default.selectAll('g path')[0];
    }
  }, {
    key: 'start',
    value: function start() {
      ...
      this.svg = _d2.default.select("body").append("svg");
      ...
      this.timer = setInterval(function () {
        return _this.dropBlocks();
      }, 500);
    }
  }, {
    key: 'dropBlocks',
    value: function dropBlocks() {
    ...
    key: 'moveBlocks',
    value: function moveBlocks(col, row) {
      var blocks = [];
      for (var i = 0; i < this.fallingblocks.length; i++) {
        ...
        this.board[x][y] = fb;
        _d2.default.select(this.positions[x * 10 + y]).attr('class', 'block ' + fb.color);
        fb.x = x;
        fb.y = y;
      ...
      for (var i = 0; i < this.fallingblocks.length; i++) {
        if (!this.fallingblocks[i].falling) this.fallingblocks.splice(i, 1);
      }
    }
  }, {
    key: 'generateBlock',
    value: function generateBlock() {
      ...
      if (!this.isEmpty(block.x, block.y)) return;
      this.fallingblocks.push(block);
      this.board[block.x][block.y] = block;
      _d2.default.select(this.positions[block.x * 10 + block.y]).attr('class', 'block ' + block.color);
    }
  }, {
    key: 'getRandomInt',
    ...
    key: 'detect3inRow',
    value: function detect3inRow(col) {
      var arr = [];
      var colBlocks = [];
      for (var i = 0; i < col.length; i++) {
        ...
        if (block.color != arr[arr.length - 1].color) arr = [];

        arr.push(block);

        if (arr.length >= 3) this.removableBlocks = this.removableBlocks.concat(arr);
      }
    }
  }, {
    key: 'detect3inColumns',
    value: function detect3inColumns(x, y) {
      var arr = [];
    ...
    value: function removeBlock(x, y) {
      ...
      _d2.default.select(this.positions[x * 10 + y]).attr('class', 'block');
    }
  }, {
    key: 'makeBlocksAboveFall',
    value: function makeBlocksAboveFall() {
      for (var n in this.removableBlocks) {
        var remBlock = this.removableBlocks[n];

        for (var i = this.board[remBlock.x].length - 1; i > 0; i--) {
          var block = this.board[remBlock.x][i];
          if (!block.empty) this.fallingblocks.push(block);
        }
      }
    }
  }, {
    key: 'increaseScore',
    value: function increaseScore(points) {
